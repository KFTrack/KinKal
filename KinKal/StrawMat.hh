...
namespace KinKal {
  class StrawMat : public DMat {
	...
	// DMat interface; first, for materials associated with a hit
      virtual void intersect(TPocaBase const& poca,std::vector<MatXing>& mxings) const override;
      // then in general
//      template <class KTRAJ> void intersect(KTRAJ const& ktraj, std::vector<MatXing>& mxings) const override;
      ...
      void intersect(float doca, float ddoca, float adot, std::vector<MatXing>& mxings) const;
