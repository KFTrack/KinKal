...
//      virtual double tension() const override { return tpoca_.doca()/sqrt(wvar_); } 
  ...
  template <class KTRAJ> void ScintHit<KTRAJ>::resid(PKTRAJ const& pktraj,  Residual& resid) const {
    // compute PTPOCA
    TPocaHint tphint( saxis_.t0(), saxis_.t0());
    PTPOCA tpoca(pktraj,saxis_,tphint);
  ...
  template <class KTRAJ> void ScintHit<KTRAJ>::update(PKTRAJ const& pktraj, MIConfig const& miconfig, Residual& residual) {
