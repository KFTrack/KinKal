...
namespace KinKal {
  class StrawHit : public WireHit {
    public:
      StrawHit(TLine const& straj, Context const& context, D2T const& d2t,StrawMat const& smat,float nulldoca, LRAmbig ambig=null,bool active=true) : 
	WireHit(straj,context,d2t,std::min(nulldoca,smat.strawRadius())*std::min(nulldoca,smat.strawRadius())/3.0,ambig,active), smat_(smat) {}
