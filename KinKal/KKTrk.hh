...
namespace KinKal {
  ...
  template <class KTRAJ> KKTrk<KTRAJ>::KKTrk(PKTRAJ const& reftraj, std::vector<const THit*> hits,Config const& config) : 
	...
	if(config_.addmat_ && hit->material() != 0){
	  auto iemp = effects_.emplace(new KKMHit<KTRAJ>(*hit,reftraj));
	  if(!iemp.second)throw std::runtime_error("Insertion failure");
	} else{ 
	  auto iemp = effects_.emplace(new KKHit<KTRAJ>(*hit,reftraj));
	  if(!iemp.second)throw std::runtime_error("Insertion failure");
	}
      }
	...
	reftraj_.setRange(TRange(std::min(reftraj_.range().low(),effects_.begin()->get()->time() - config_.tbuff_),
      ...
      // create end effects
      auto iemp = effects_.emplace(new KKEnd<KTRAJ>(reftraj,TDir::forwards,config_.dwt_));
      if(!iemp.second)throw std::runtime_error("Insertion failure");
      iemp = effects_.emplace(new KKEnd<KTRAJ>(reftraj,TDir::backwards,config_.dwt_));
      if(!iemp.second)throw std::runtime_error("Insertion failure");
    }
