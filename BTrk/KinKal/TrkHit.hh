...
namespace KinKal {
  ...
  template <unsigned HDIM=1> struct Residual {
    public:
      typedef ROOT::Math::SVector<double,HDIM> RVec; // residual vector type
      typedef ROOT::Math::SMatrix<double,HDIM,HDIM,ROOT::Math::MatRepSym<double,HDIM> > RMat;  // associated covariance type
  ...
  template <unsigned HDIM=1> class TrkHit {
    public:
      unsigned nCons() const { return HDIM; }
      ...
      virtual bool resid(TPOCABase const& tpoca, Resid& resid, RDer const& dRdDT, double nsigma) const =0;
