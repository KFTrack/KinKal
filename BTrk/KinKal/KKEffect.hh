...
namespace KinKal {
  ...
  template<class KTRAJ> class KKEffect {
    public:
      ...
      typedef typename WData<KTRAJ::PDATA::PDim()> WDATA; // declare the associated weight type as well 
    ...
    protected:
      KKEffect(KTRAJ const& reftraj) : reftraj_(reftraj), status_(unprocessed), active_(true) {}
  ...
  void template<> KKEffect<KTRAJ>::Update(KTraj const& ref) { reftraj_ = ref; }
