...
unsigned nhits(40);
double escale(5.0);
vector<double> sigmas = {3.0,  3.0, 3.0, 3.0,
                         0.03, 3.0}; // base sigmas for parameters (per hit!)
// define the context
...
void print_usage() {
  printf("Usage: FitTest  --momentum f --costheta f --azimuth f --particle i "
         "--charge i --zrange f --nhits i --hres f --seed i --escale f "
         "--maxniter f --ambigdoca f --ntries i --addmat i\n");
}
...
KinKal::TLine GenerateStraw(PLHelix const &traj, double htime) {
  ...
  double shlen =
      sqrt(rmax * rmax + ddot * ddot - drho * drho); // straw half-length
  // choose the closest end to be the measurement end
  double rprop = (fabs(-ddot - shlen) < fabs(-ddot + shlen)) ? -ddot - shlen
                                                             : -ddot + shlen;
...
void createHits(PLHelix &plhel, StrawMat const &smat,
                std::vector<StrawHit> &shits, bool addmat) {
  ...
  for (size_t ihit = 0; ihit < nhits; ihit++) {
    ...
    if (addmat && simmat) {
      ...
      for (int idir = 0; idir <= KInter::theta2; idir++) {
        ...
        // generate a random effect given this variance and mean.  Note
        // momEffect is scaled to momentum
        switch (mdir) {
        case KinKal::KInter::theta1:
        case KinKal::KInter::theta2:
          dm = TR->Gaus(dmom, sqrt(momvar));
          dscatsum += dm;
          break;
        case KinKal::KInter::momdir:
          //	    dm = -TR->Landau(-dmom,sqrt(momvar)); // have to flip sign
          // for root!
          dm = dmom;
          desum += dm * mom;
          break;
        ...
        //	cout << "direction " << mdir << " doca " << tp.doca() << " dmom
        //" << dmom << " +- " << sqrt(momvar) << " sample " << dm  << endl;
        Vec3 dmvec;
        endpiece.dirVector(mdir, tp.t0(), dmvec);
        dmvec *= dm * mom;
        //	dmvec *= 0.0;
        endmom.SetCoordinates(endmom.Px() + dmvec.X(), endmom.Py() + dmvec.Y(),
                              endmom.Pz() + dmvec.Z(), endmom.M());
      }
      // generate a new piece and append
      LHelix newend(endpos, endmom, endpiece.charge(), context,
                    TRange(tp.t0(), plhel.range().high()));
      if (!plhel.append(newend))
        cout << "Error appending traj " << newend << endl;
    }
  }
  //  cout << "Total energy loss " << desum << " scattering " << dscatsum <<
  //  endl;
}
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long_only(argc, argv, "", long_options, &long_index)) !=
    ...
    case 'b':
      ...
      break;
    case 'd':
      ambigdoca = atof(optarg);
      break;
    case 't':
      ntries = atoi(optarg);
      break;
    case 'i':
      mindchisq = atof(optarg);
      break;
    default:
      print_usage();
      exit(EXIT_FAILURE);
  ...
  Vec3 vel;
  plhel.velocity(0.0, vel);
  plhel.setRange(
      TRange(-0.5 * zrange / vel.Z() - tbuff, 0.5 * zrange / vel.Z() + tbuff));
  // generate hits
  ...
  cout << "total momentum change = " << startmom - endmom
       << " total angle change = " << angle << endl;
  // create the fit seed by randomizing the parameters at the middle
  auto seedhel = plhel.nearestPiece(0.0);
  createSeed(seedhel);
  cout << "Seed params " << seedhel.params().parameters() << " covariance "
       << endl
       << seedhel.params().covariance() << endl;
  ...
  for (auto const &eff : effs) {
    cout << "Eff at time " << eff->time() << " status "
         << eff->status(TDir::forwards) << " " << eff->status(TDir::backwards);
    auto ihit = dynamic_cast<const KKHit<LHelix> *>(eff.get());
    auto imhit = dynamic_cast<const KKMHit<LHelix> *>(eff.get());
    if (ihit != 0) {
      cout << " Hit status " << ihit->poca().status() << " doca "
           << ihit->poca().doca() << ihit->refResid() << endl;
    } else if (imhit != 0) {
      cout << " MHit status " << imhit->hit().poca().status() << " doca "
           << imhit->hit().poca().doca() << imhit->hit().refResid() << endl;
    } else
      cout << endl;
  }
  ...
  if (ntries <= 0) {
    ...
    TH1F *chisqndof =
        new TH1F("chisqndof", "Chisquared per NDOF", 100, 0, 10.0);
    TH1F *chisqprob =
        new TH1F("chisqprob", "Chisquared probability", 100, 0, 1.0);
    TH1F *logchisqprob =
        new TH1F("logchisqprob", "Chisquared probability", 100, -10, 0.0);
    string htitle, hname;
    TH2F *corravg = new TH2F("corravg", "Average correlation matrix magnitudes",
                             LHelix::NParams(), -0.5, LHelix::NParams() - 0.5,
                             LHelix::NParams(), -0.5, LHelix::NParams() - 0.5);
    ...
    for (size_t ipar = 0; ipar < LHelix::NParams(); ipar++) {
      ...
      dpgenh[ipar] = new TH1F(hname.c_str(), htitle.c_str(), 100,
                              -pscale * sigmas[ipar], pscale * sigmas[ipar]);
      hname = string("p") + LHelix::paramName(tpar);
      htitle = string("Pull ") + LHelix::paramTitle(tpar);
      dpullgenh[ipar] =
          new TH1F(hname.c_str(), htitle.c_str(), 100, -nsig, nsig);
      hname = string("e") + LHelix::paramName(tpar);
      htitle = string("Error ") + LHelix::paramTitle(tpar);
      fiterrh[ipar] = new TH1F(hname.c_str(), htitle.c_str(), 100, 0.0,
                               pscale * sigmas[ipar]);
    ...
    for (unsigned itry = 0; itry < ntries; itry++) {
      // randomize the helix
      Vec4 torigin(TR->Gaus(0.0, 3.0), TR->Gaus(0.0, 3.0), TR->Gaus(0.0, 3.0),
                   TR->Gaus(0.0, 3.0));
      double tphi = TR->Uniform(-M_PI, M_PI);
      double tcost = TR->Uniform(0.5, 0.8);
      double tsint = sqrt(1.0 - tcost * tcost);
      Mom4 tmomv(mom * tsint * cos(tphi), mom * tsint * sin(tphi), mom * tcost,
                 pmass);
      LHelix tlhel(torigin, tmomv, icharge, context);
      PLHelix tplhel(tlhel);
      Vec3 vel;
      tplhel.velocity(0.0, vel);
      tplhel.setRange(TRange(-0.5 * zrange / vel.Z() - tbuff,
                             0.5 * zrange / vel.Z() + tbuff));
      shits.clear();
      ...
      for (size_t ipar = 0; ipar < LHelix::NParams(); ipar++) {
        ...
        dpullgenh[ipar]->Fill(
            (fpars.parameters()[ipar] - tpars.parameters()[ipar]) / cerr[ipar]);
