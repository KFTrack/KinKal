...
void print_usage()
{
  printf("Usage: IPHelix  --momentum f --costheta f --azimuth f --particle i --charge i --zorigin f --torigin --tmin f--tmax f --ltime f \n");
}
...
int main(int argc, char **argv)
{
  ...
  printf("Testing IPHelix with momentum = %f, costheta = %f, phi = %f, mass = %f, charge = %i, z = %f, t = %f \n", mom, cost, phi, pmass, icharge, oz, ot);

  Vec3 bnom(0.0, By, 1.0);
  Vec4 origin(0.0, 0.0, oz, ot);
  float sint = sqrt(1.0 - cost * cost);
  Mom4 momv(mom * sint * cos(phi), mom * sint * sin(phi), mom * cost, pmass);
  std::cout << "Momv " << momv.Px() << " " << momv.Py() << " " << momv.Pz() << std::endl;
  IPHelix HHel(origin, momv, icharge, bnom);
  Mom4 testmom;
  HHel.momentum(ot, testmom);
  cout << "IPHelix with momentum " << testmom << " position " << origin << " has parameters: " << HHel << endl;
  Vec3 vel;
  HHel.velocity(ot, vel);
  double dot = vel.Dot(testmom) / CLHEP::c_light;
  cout << "velocity dot mom = " << dot << endl;
  cout << "momentum beta =" << momv.Beta() << " IPHelix beta = " << HHel.beta() << endl;
  Vec3 mdir;
  HHel.direction(ot, mdir);
  // create the helix at tmin and tmax
  Mom4 tmom;
  Vec4 tpos;
  HHel.momentum(tmax, tmom);
  tpos.SetE(tmax);
  HHel.position(tpos);
  IPHelix HHelmax(tpos, tmom, icharge, bnom);
  HHel.momentum(tmin, tmom);
  tpos.SetE(tmin);
  HHel.position(tpos);
  IPHelix HHelmin(tpos, tmom, icharge, bnom);

  cout << "IPHelix at tmax has parameters : " << HHelmax << endl;
  cout << "IPHelix at tmin has parameters : " << HHelmin << endl;
  ...
  Vec3 mompos;
  HHel.position(ot, mompos);
  HHel.direction(ot, mdir);
  Vec3 momvec = mom * mdir;
  drawMom(mompos, momvec, kBlack, mref);
  //
  HHel.position(tmin, mompos);
  HHel.direction(tmin, mdir);
  momvec = mom * mdir;
  drawMom(mompos, momvec, kBlue, mstart);
  //
  HHel.position(tmax, mompos);
  HHel.direction(tmax, mdir);
  momvec = mom * mdir;
  drawMom(mompos, momvec, kGreen, mend);
  //
  TLegend *leg = new TLegend(0.8, 0.8, 1.0, 1.0);
  char title[80];
  snprintf(title, 80, "IPHelix, q=%1i, mom=%3.1g MeV/c", icharge, mom);
  ...
  Vec3 pos, dir;
  HHel.position(ltime, pos);
  HHel.direction(ltime, dir);
  // // rotate the direction
  // double HHphi = atan2(dir.Y(), dir.X());
  // double pphi = HHphi + M_PI / 2.0;
  ...
  // // find TPOCA
  // TPOCA<IPHelix, TLine> tp(HHel, tline);
  ...
  snprintf(title, 80, "IPHelix_m%3.1f_p%3.2f_q%i.root", pmass, mom, icharge);
