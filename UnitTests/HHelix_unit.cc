...
void print_usage()
{
  printf("Usage: HHelix  --momentum f --costheta f --azimuth f --particle i --charge i --zorigin f --torigin --tmin f--tmax f --ltime f \n");
}
...
int main(int argc, char **argv)
{
  ...
  printf("Testing HHelix with momentum = %f, costheta = %f, phi = %f, mass = %f, charge = %i, z = %f, t = %f \n", mom, cost, phi, pmass, icharge, oz, ot);
  // define the context
...
// define the context
  ...
  HHelix HHel(origin, momv, icharge, context);
  Mom4 testmom;
  ...
  tpos.SetE(tmax);
  HHel.position(tpos);
  HHelix HHelmax(tpos, tmom, icharge, context);
  HHel.momentum(tmin, tmom);
  tpos.SetE(tmin);
  HHel.position(tpos);
  HHelix HHelmin(tpos, tmom, icharge, context);

  cout << "HHelix at tmax has parameters : " << HHelmax << endl;
  cout << "HHelix at tmin has parameters : " << HHelmin << endl;
  ...
  char title[80];
  snprintf(title, 80, "HHelix, q=%1i, mom=%3.1g MeV/c", icharge, mom);
  ...
  // // find TPOCA
  // TPOCA<HHelix, TLine> tp(HHel, tline);
  ...
  snprintf(title, 80, "HHelix_m%3.1f_p%3.2f_q%i.root", pmass, mom, icharge);
