...
void print_usage() {
  printf("Usage: HHelixDerivs  --momentum f --costheta f --azimuth f --particle i --charge i --zorigin f --torigin --dmin f --dmax f --ttest f\n");
}
...
int main(int argc, char **argv) {
  ...
  HHelix refhel(origin,momv,icharge,context);
...
//      cout << "Delta = " << delta << endl;
      ...
      HHelix xhel(refpos,momv,icharge,context);
...
//      cout << "derivative vector" << pder << endl;
      ...
      //dvec += pder;
      HHelix dhel(dvec, refhel.params().covariance(), refhel.mass(), refhel.charge(), context);
      // test
      Vec4 xpos, dpos;
      xpos.SetE(ttest);
      dpos.SetE(ttest);
